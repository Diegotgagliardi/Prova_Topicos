...
app.MapPut("/servicos/{id}", async (int id, Servico servico, ServicoService servicoService) =>
{
    if (id != servico.Id)
    {
        return Results.BadRequest("ID do serviço não corresponde.");
    }

    // Obter o serviço existente do banco de dados
    var servicoExistente = servicoService.GetServicoById(id);
    if (servicoExistente == null)
    {
        return Results.NotFound($"Serviço com ID {id} não encontrado.");
    }

    // Atualizar os atributos do serviço existente
...
// Rota autenticada para registrar um contrato
app.MapPost("/contratos", async (Contrato contrato, ContratoService contratoService) =>
{
    // Validar se o cliente e o serviço existem
    var cliente = await contratoService.GetClienteByIdAsync(contrato.ClienteId);
    var servico = await contratoService.GetServicoByIdAsync(contrato.ServicoId);
    ...
    // Registrar o contrato no banco de dados
    var novoContrato = await contratoService.AddContratoAsync(contrato);
    if (novoContrato == null)
    {
        return Results.BadRequest("Erro ao registrar o contrato.");
    }

    return Results.Created($"/contratos/{novoContrato.Id}", novoContrato);
}).RequireAuthorization();

// Rota autenticada para consultar os serviços contratados por um cliente
app.MapGet("/clientes/{clienteId}/servicos", async (int clienteId, ContratoService contratoService) =>
{
    ...
    {
        return Results.NotFound($"Nenhum serviço encontrado para o cliente com ID {clienteId}.");
    }

    // Mapear contratos para serviços (usando ServicoId)
    var servicosContratados = new List<Servico>();
